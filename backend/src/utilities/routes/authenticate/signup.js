...
const schema = joi.object({
 ...
 function SignUp(data){
    ...
    return new Promise(async (resolve,reject)=>{
        if(error){
           return reject({status:422,message:error.details})
        }
        else{
            ...
            if(email){return reject({status:409,message:"Email already exists!"})}
                ...
                let password = await Decrypt(value.password).then(result =>{return(result)}).catch(error =>{console.log(error)})
                ...
                if(newUser){
                    return resolve('User has been created successfully');
                }else{
                   return reject({status:400,message:"User cannot be created!"})
                }
        }
    })
